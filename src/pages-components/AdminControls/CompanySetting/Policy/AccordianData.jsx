...
const LivePreviewExample = (props) => {
  ...
  const handlPolicy = (options) => {
    setSelectedPolicy(options.value);
  };

  const policyList = dropDownPolicies?.map((pl) => {
    return { value: pl.id_policy, label: pl.name_policy };
  });
  ...
  // console.log("dropDownPolicies", dropDownPolicies);
  return (
    <>
      <div className="accordion mb-5">
        {props.data?.map((data, key) => {
          return (
            data.is_benefit === null && (
              <Card
                key={key}
